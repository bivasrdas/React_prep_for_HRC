...
componentDidUpdate()
{
    ...
    voiceCommands()
    {
        var transcript;
        recognition.start()
        recognition.onstart=()=>{console.log("Voice activated");alert("Voice activated")}
        ...
        recognition.onend=()=>{console.log("Voice Deactivated");alert("Voice Deactivated")}
    ...
    <div className="name" style={{color:"white",paddingLeft:"10%", fontSize:"50px",fontWeight:"500",fontStretch:"ex"}}>{this.state.dialog}</div>
    ...
    fontWeight:"400",marginTop:"10px"}}><div id="deaths"></div></div></div></Paper>
        ...
        <Button id="voiceb" onClick={this.voiceCommands} ><div styles={{ backgroundColor:"white" }} className="interact" >activate</div></Button>
        {/*<h2 id="interm"></h2>*/}
        <h1 styles={{ color:"white" }}>{this.state.nlpcontent}</h1>
    <h3>{this.state.dialog}</h3>
        </div>)
